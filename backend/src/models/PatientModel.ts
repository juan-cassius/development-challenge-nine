...
export default class PatientModel implements IPatientModel {
  ...
  async create(data: NewEntity<IPatientWithAddress>): Promise<IPatientWithAddress> {
    ...
    const {
      id, fullName, birthDate, email,
    } = dbData;
    const convertedData: IPatientWithAddress = {
      id, fullName, birthDate, email, address: data.address,
    };
  ...
  async findAll(): Promise<IPatientWithAddress[]> {
    ...
    const convertedData: IPatientWithAddress[] = dbData.map(({
      id, fullName, birthDate, email, address,
    }: IPatient | any) => ({
      id, fullName, birthDate, email, address,
    }));
  ...
  async findById(id: number): Promise<IPatientWithAddress | null> {
    ...
    const {
      fullName, birthDate, email, address,
    }: IPatient | any = dbData;
    const convertedData: IPatientWithAddress = {
      id, fullName, birthDate, email, address,
    };
  ...
  async update(id: number, data: Partial<NewEntity<IPatientWithAddress>>):
        Promise<IPatientWithAddress | null> {
