...
export default class PatientService {
  ...
  public async updatePatient(id: number, patient: IPatientWithAddress):
        Promise<ServiceResponse<ServiceMessage>> {
    const patientFound = await this.patientModel.findById(id);

    if (!patientFound) return { status: 'NOT_FOUND', data: { message: 'Patient not found' } };
    ...
    if (!updatePatient && !updateAddress) {
      return {
        status: 'CONFLICT',
        data: { message: 'There are no updates to perform in this patient or his address' },
      };
    }
